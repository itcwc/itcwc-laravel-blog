...
<article class="content-item pt-8 group relative" data-keywords="{{ $item->keywords }}">
    <header class="mb-6">
        <span class="text-[10px] font-mono text-zinc-300 uppercase tracking-widest">{{ __('content.article_label') }} {{ str_pad($contents->total() - ($contents->firstItem() + $index) + 1, 2, '0', STR_PAD_LEFT) }}</span>
    ...
    <p class="text-[10px] font-mono text-zinc-300 uppercase tracking-widest mb-6">
        {{ $item->published_date->format('Y.m.d') }} · {{ $item->read_time ?? '5' }} {{ __('content.read_time') }}
    </p>
    ...
    <footer class="flex gap-8 text-[11px] tracking-[0.2em] uppercase">
        <a href="/article/{{ $item->slug ?? $item->id }}" class="border-b border-zinc-900 pb-1 hover:text-zinc-400 hover:border-zinc-400 transition-all duration-300">{{ __('ui.read_full') }}</a>
    </footer>

    @auth
    <div class="absolute top-8 right-0 opacity-0 group-hover:opacity-100 transition-all">
        <button data-item="{{ json_encode($item) }}" onclick="handleEdit(this)" class="px-3 py-1 bg-black text-[9px] text-white tracking-[0.2em] uppercase shadow-xl">{{ __('ui.edit') }}</button>
        <button onclick="confirmGeneralDelete(`{{ $item->id }}`, this.closest('.content-item'))" class="bg-red-500 text-white px-3 py-1 text-[10px] tracking-widest uppercase hover:bg-red-600 transition">{{ __('ui.delete') }}</button>
    </div>
    @endauth
...
<div class="note-item pt-8 group relative " data-keywords="{{ $item->keywords }}">
    ...
    <div class="absolute top-8 right-0 opacity-0 group-hover:opacity-100 transition-all">
        <button data-item="{{ json_encode($item) }}" onclick="handleEdit(this)" class="px-3 py-1 bg-black text-[9px] text-white tracking-[0.2em] uppercase shadow-xl">{{ __('ui.edit') }}</button>
        <button onclick="confirmGeneralDelete(`{{ $item->id }}`, this.closest('.note-item'))" class="bg-red-500 text-white px-3 py-1 text-[10px] tracking-widest uppercase hover:bg-red-600 transition">{{ __('ui.delete') }}</button>
    </div>
    @endauth
    <hr class="mt-8 border-zinc-100">
</div>
